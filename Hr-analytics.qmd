---
title: "Аналитика зарплат 3D Motion Designer"
format: 
  html:
    page-layout: full
    code-fold: false
    code-tools: false
    html-math-method: katex
    toc: true
    toc-location: right
css: styles.css
---

Зарплатная аналитика по вакансиям и резюме.

**Должность:** 3D Motion Designer.

**Источники данных:** hh.ru, телеграм каналы и чаты для геймдева.

**Регионы:** Россия, Польша, Черногория, Литва, Турция, Эстония, Армения, Словакия, Азербайджан, Кыргызстан, Латвия, Чехия, Таджикистан, Казахстан, Словения, Молдова, Грузия, Кипр, Узбекистан, Сербия

```{r}
#| warning: false
#| echo: false
library(tidyverse)
library(scales)
library(kableExtra)
a <- read_csv("3D_Motion_Resume_04.2024.csv")
```

### Резюме

::: {.panel-tabset}

#### Колличество данных

```{r}
#| warning: false
#| echo: false
p1 <- colSums(!is.na(a[1]))
p2 <- colSums(!is.na(a[2]))
p3 <- colSums(!is.na(a[3]))
p4 <- colSums(!is.na(a[4]))
p5 <- sum(a$Офис, na.rm = T)
p6 <- sum(a$Удалённо, na.rm = T)
p7 <- sum(a$Гибрид, na.rm = T)

p <- tibble("Junior" = p1, "Middle" = p2, "Senior" = p3, "Team Lead" = p4, "Офис" = p5, "Удалённо" = p6, "Гибрид" = p7)

kbl(p) |> 
  kable_material()
```

#### Таблица данных

```{r}
#| warning: false
#| echo: false
kbl(a) |> 
  kable_material(c("striped", "hover")) |> 
  kableExtra::scroll_box(height = "400px")
```

:::

```{r}
#| warning: false
#| echo: false
#| out-width: 100%
a1 <- a |> 
  select("Junior", "Middle", "Senior", "Team Lead")
  
a1 <- gather(a1)

ggplot(a1, mapping = aes(key, value, color = key)) +
  geom_boxplot(outlier.colour = "red", notch=FALSE) +
  geom_jitter(shape=16, position=position_jitter(0.2)) +
  facet_grid() +
  scale_color_brewer(palette="Set2") +
  theme_minimal() +
  labs(x = " ") +
  labs(y = " ") +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  scale_y_continuous(labels = comma) +
  guides(color = FALSE)
```

```{r}
#| warning: false
#| echo: false
#a <- drop_na(a, Junior, Middle, Senior, `Team Lead`)
knitr::kable(summary(a[1:4]))
```

---

### Вакансии

```{r}
#| warning: false
#| echo: false
junior <- read_csv("3D_Motion_Vacancy_Junior_04.2024.csv")
middle <- read_csv("3D_Motion_Vacancy_Middle_04.2024.csv")
senior <- read_csv("3D_Motion_Vacancy_Senior_04.2024.csv")
teamlead <- read_csv("3D_Motion_Vacancy_TeamLead_04.2024.csv")
```

::: {.panel-tabset}

#### Колличество данных

|    | Зарплата | ДМС | Обучение | Фитнес |
|:--:|:--------:|:---:|:--------:|:------:|
| Junior| 1 | 0 | 0 | 0 |
| Middle | 2 | 0 | 0 | 0 |
| Senior | 3 | 1 | 0 | 1 |
| Team Lead | 0 | 0 | 0 | 0 |

#### Junior

```{r}
#| warning: false
#| echo: false
kbl(junior) |> 
  kable_minimal("hover") |> 
  kableExtra::scroll_box(height = "400px")
```

#### Middle

```{r}
#| warning: false
#| echo: false
kbl(middle) |> 
  kable_minimal() |> 
  kableExtra::scroll_box(height = "400px")
```

#### Senior

```{r}
#| warning: false
#| echo: false
kbl(senior) |> 
  kable_minimal() |> 
  kableExtra::scroll_box(height = "400px")
```

#### Team Lead

```{r}
#| warning: false
#| echo: false
kbl(teamlead) |> 
  kable_minimal() |> 
  kableExtra::scroll_box(height = "400px")
```

:::

```{r}
#| warning: false
#| echo: false
#| out-width: 100%

j1 <- gather(junior[ ,1])
m1 <- gather(middle[ ,1])
s1 <- gather(senior[ ,1])
t1 <- gather(teamlead[ ,1])

j1$key <- paste0("Junior")
m1$key <- paste0("Middle")
s1$key <- paste0("Senior")
t1$key <- paste0("Team Lead")

q <- rbind2(j1, m1, by="key", all.x=T, sort=F)
q1 <- rbind2(s1, t1, by="key", all.x=T, sort=F)
q2 <- rbind2(q, q1)

ggplot(q2, mapping = aes(key, value, color = key)) +
  geom_boxplot(outlier.colour = "red", notch=FALSE) +
  geom_jitter(shape=16, position=position_jitter(0.2)) +
  facet_grid() +
  scale_color_brewer(palette="Set2") +
  theme_minimal() +
  labs(x = " ") +
  labs(y = " ") +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  scale_y_continuous(labels = comma) + 
  guides(color = FALSE)
```

```{r}
#| warning: false
#| echo: false

j <- summary(j1[2])
m <- summary(m1[2])
s <- summary(s1[2])
t <- summary(t1[2])

vac <- tibble("Junior" = j, "Middle" = m, "Senior" = s, "Team Lead" = t)

kbl(vac) |> 
  kable_material() |> 
  kableExtra::scroll_box(height = "400px")
```